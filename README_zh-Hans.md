...
项目也可以监控官方 AI 服务状态，只推送有意义的故障变化和恢复。按照 [`env.example`](env.example) 中服务状态变量旁的注释即可启用和配置。
...
`LLM_EXTRA_HEADERS` 和 `LLM_FALLBACK_EXTRA_HEADERS` 可选地接受单行 JSON 对象，其中名称和值都必须是 HTTP header 字符串。备用 header 只有在备用提供商和模型均已配置时才能使用。应将这些值视为凭据：配置项会覆盖 SDK 默认值，包括 `Authorization` 和 `User-Agent`，应用不会把它们写入日志。
...
常驻调度器默认每天 08:00 发送预报，并在 09:00&ndash;23:00 检查天气变化。具体时区和调度策略均可在 `.env` 中调整。
