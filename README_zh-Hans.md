...
官方 AI 服务状态不需要状态页凭据，环境变量模板默认关闭；将 `SERVICE_STATUS_PROVIDERS` 设为 `deepseek,openai,anthropic,kimi` 的逗号分隔子集即可开启。它与天气简报独立地每五分钟运行一次，每条官方消息都会先判断是否值得提醒，只有有意义的故障变化和恢复才会推送。消息已经是英语或用户指定语言时原样转发，其他语言才使用大语言模型忠实翻译。`SERVICE_STATUS_CRON` 设置独立的五段 cron，`SERVICE_STATUS_LANGUAGE` 可选择 `en`、`zh-CN` 或 `ja`，`SERVICE_STATUS_PUBLISHERS` 可用逗号指定多个投递平台，默认沿用 `PUBLISHER`。
...
常驻调度器默认每天 08:00 发送预报，并在 09:00&ndash;23:00 检查天气变化。配置 `SERVICE_STATUS_PROVIDERS` 后，调度器还会按独立策略每五分钟轮询相应服务；留空只会关闭服务状态任务。具体时区、`BRIEFING_CRON` 和 `SERVICE_STATUS_CRON` 均可在 `.env` 中调整。
