...
不配置两个加密变量时，Bark 会发送明文；官方提供的另一种隐私保护方案是部署自己的 Bark 服务端，并通过 `BARK_BASE_URL` 配置其根 URL，默认值是 `https://api.day.app`。启用加密时必须同时配置两个变量，并在 App 的“推送加密”中根据 `BARK_ENCRYPTION_KEY` 的长度选择 AES128、AES192 或 AES256。加密 key 必须由 16、24 或 32 个 ASCII 字符组成；模式选择 GCM，padding 选择 noPadding，并在 App 中保存与 `BARK_ENCRYPTION_IV` 相同的 12 字符固定 IV。Bark 要求固定使用该 IV，而 AES-GCM 重复使用 IV 会削弱安全性，因此应把 Bark 加密视为防止无意泄露，而不是强认证信道。CBC 和 ECB 不验证密文完整性，因此项目有意不支持。Bark 简报最多包含 650 个可见字符，使内容和元数据保持在 APNs payload 限制内。

`BARK_GROUP` 控制通知分组，默认值是 `weather-briefing`。普通投递使用 Bark 的 `timeSensitive` 时效级别，应用生成的静默投递使用 `passive`。
