...
Docker 是一种部署方式。下面的示例使用 Docker Hub 上的固定版本镜像；也可以用其他方式运行项目，只要它能长期运行程序并保存上述配置和状态。
...
`language` 控制该地点简报的语言，默认是英文 `en`。支持的取值为 `en`（默认）、`ja`、`zh-CN` 和 `zh-TW`。日本地点如需 JMA 预报，还要填写当地六位 `jma_office_code`。
...
首次使用 Telegram 投递时，请先在 Telegram 中打开该机器人并向它发送 `/start` 消息。机器人只有在用户主动启动会话后，才能向对应的 Chat ID 发送消息；否则天气简报会因目标会话不可达而投递失败。
