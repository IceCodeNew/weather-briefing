...
官方 AI 服务状态默认开启，不需要状态页凭据，并与天气简报独立地每五分钟运行一次。每条官方消息都会先判断是否值得提醒，只有有意义的故障变化和恢复才会推送。消息已经是英语或用户指定语言时原样转发，其他语言才使用大语言模型忠实翻译。`SERVICE_STATUS_PROVIDERS` 用于选择 `deepseek,openai,anthropic,kimi` 的子集，留空可关闭；`SERVICE_STATUS_CRON` 设置独立的五段 cron，`SERVICE_STATUS_LANGUAGE` 可选择 `en`、`zh-CN` 或 `ja`，`SERVICE_STATUS_PUBLISHERS` 可用逗号指定多个投递平台，默认沿用 `PUBLISHER`。
