...
フォールバックはデフォルトで無効です。両方のフォールバック変数を設定すると、プライマリープロバイダーのリクエスト失敗時にフォールバックで再試行します。最初のリクエスト失敗後は、現在の実行中の修復や後続地点でもフォールバックを使い続けます。次回のスケジュール実行または CLI 実行では、プライマリープロバイダーを再び試します。構造化出力契約に違反する応答はプロバイダーを切り替えず、通常の回数制限付き修復を行います。
