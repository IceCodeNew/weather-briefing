...
地点名のみ指定した場合は、プログラムが座標を検索して `locations.json` に書き戻します。座標のみ指定した場合は、読みやすい地点名を逆引きして書き戻します。既存のフィールドは上書きしません。精度を下げて得た一致候補は引き続き確認が必要で、自動では書き込みません。両方指定した場合は、ジオコーディング呼び出しは行われません。
