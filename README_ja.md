...
常駐スケジューラはデフォルトで毎日 08:00 に天気予報を送信し、09:00&ndash;23:00 に天気の変化を確認し、有効な AI サービスのステータスを5分ごとに確認します。天気とサービスステータスのタスクは同時に有効化できますが、スケジュールは独立しています。タイムゾーン、`BRIEFING_CRON`、`SERVICE_STATUS_CRON` は `.env` で調整できます。
