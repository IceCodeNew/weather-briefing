...
- Telegram または Bark の配信認証情報。Telegram には Bot Token と Chat ID、Bark にはデバイスキーが必要で、暗号化キーと IV は任意です。
...
2 つの暗号化変数を設定しない場合、Bark は平文で送信します。公式が提供するもう一つのプライバシー保護方法は、独自の Bark サーバーをデプロイし、そのルート URL を `BARK_BASE_URL` に設定することです。デフォルトは `https://api.day.app` です。暗号化を有効にする場合は両方の変数を設定し、App の「プッシュ暗号化」で `BARK_ENCRYPTION_KEY` の長さに合わせて AES128、AES192、AES256 のいずれかを選択します。暗号化キーは 16、24、32 文字の ASCII 文字列にし、モードは GCM、padding は noPadding を選び、同じキーと 12 文字の固定 `BARK_ENCRYPTION_IV` を App に保存してください。Bark はこの IV の固定使用を要求しますが、AES-GCM で IV を再利用すると安全性が低下するため、Bark の暗号化は不用意な漏洩を防ぐものと位置付け、強力な認証済みチャネルとは見なしません。CBC と ECB は暗号文の完全性を検証できないため、意図的に対応していません。Bark ブリーフィングは、内容とメタデータを APNs の payload 上限内に収めるため、表示文字数を 650 文字に制限します。

`BARK_GROUP` は通知のグループを制御し、デフォルトは `weather-briefing` です。通常の配信には Bark の `timeSensitive` レベルを使用し、アプリケーションが生成するサイレント配信には `passive` を使用します。
