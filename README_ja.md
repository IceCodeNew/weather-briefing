...
Weather Briefing は、天気・大気質・警報・任意のプライベート RSS を定期的に集約し、大規模言語モデルで出典付きのブリーフィングを生成します。公式 AI サービスのステータスは独立して監視し、障害の変化と復旧を直接通知します。
...
- DeepSeek、OpenAI、Anthropic、Kimi の公式ステータスページを独立して監視し、Web サービスと API サービスを区別します。
...
公式 AI サービスのステータス監視にステータスページ用の認証情報は不要です。環境変数テンプレートでは無効になっており、`SERVICE_STATUS_PROVIDERS` に `deepseek,openai,anthropic,kimi` の一部をカンマ区切りで指定すると有効になります。天気ブリーフィングとは独立して5分ごとに実行し、公式メッセージごとに通知する価値があるかを判断して、意味のある障害変化と復旧だけを通知します。英語または指定言語のメッセージはそのまま転送し、それ以外だけ大規模言語モデルで忠実に翻訳します。独立した5フィールド cron は `SERVICE_STATUS_CRON`、通知言語は `SERVICE_STATUS_LANGUAGE`、複数の配信先はカンマ区切りの `SERVICE_STATUS_PUBLISHERS` で設定でき、未指定時は `PUBLISHER` を使用します。
...
常駐スケジューラはデフォルトで毎日 08:00 に天気予報を送信し、09:00&ndash;23:00 に天気の変化を確認します。`SERVICE_STATUS_PROVIDERS` を設定すると、独立したスケジュールで対象サービスを5分ごとに確認し、空にするとサービスステータスのタスクだけを無効にします。タイムゾーン、`BRIEFING_CRON`、`SERVICE_STATUS_CRON` は `.env` で調整できます。
