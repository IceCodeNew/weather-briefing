...
Bark は、暗号化キーと IV を設定しなければ平文で送信します。

別のプライバシー保護方法として Bark サーバーをセルフホストし、その URL を `BARK_BASE_URL` に設定できます。デフォルトは `https://api.day.app` です。

暗号化する場合は、`BARK_ENCRYPTION_KEY` と `BARK_ENCRYPTION_IV` を両方設定します。

Bark App の「プッシュ暗号化」で、キーの長さに応じて AES128、AES192、AES256 のいずれかを選択し、モードは GCM、padding は noPadding に設定します。キーは16文字、24文字、32文字のいずれかとし、IV には12文字の固定値を指定してください。

Bark の仕様では IV が固定されるため、AES-GCM の安全性は低下します。この暗号化は不用意な漏洩を防ぐためのもので、強力な認証済みチャネルではありません。暗号文の完全性を検証できない CBC と ECB には対応していません。

Bark ブリーフィングは、APNs の payload 上限に収まるよう、650文字までに制限します。

`BARK_GROUP` で通知をグループ化できます。デフォルトは `weather-briefing` です。通常の通知は `timeSensitive`、サイレント通知は `passive` として送信されます。
