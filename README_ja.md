...
フォールバックはデフォルトで無効です。両方のフォールバック変数を設定すると、プライマリープロバイダーのリクエスト失敗時にフォールバックで再試行します。最初のリクエスト失敗後は、プロセスが再起動するまでフォールバックを使い続けます。構造化出力契約に違反する応答はプロバイダーを切り替えず、通常の回数制限付き修復を行います。
