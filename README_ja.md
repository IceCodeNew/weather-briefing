...
本プロジェクトは公式 AI サービスのステータスも監視し、意味のある障害変化と復旧だけを通知できます。有効化と設定は [`env.example`](env.example) のサービスステータス変数に付いたコメントに従ってください。
...
`LLM_EXTRA_HEADERS` と `LLM_FALLBACK_EXTRA_HEADERS` には、HTTP ヘッダー名と文字列値からなる一行の JSON オブジェクトを任意で設定できます。フォールバック用ヘッダーにはフォールバックプロバイダーとモデルの設定が必要です。値は認証情報として扱ってください。設定したフィールドは `Authorization` や `User-Agent` を含む SDK のデフォルトを上書きし、アプリケーションはログへ出力しません。
...
常駐スケジューラはデフォルトで毎日 08:00 に天気予報を送信し、09:00&ndash;23:00 に天気の変化を確認します。タイムゾーンとスケジュールは `.env` で調整できます。
