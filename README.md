...
- 一台能运行 Docker 的长期在线主机；
- Telegram Bot Token 和接收消息的 Chat ID；
- 一个受支持的大语言模型账号、模型名称和凭据；
- 至少一个关注地点；
- 一个可持久保存 SQLite 状态和定位缓存的目录。

默认天气服务不需要密钥。中国大陆用户如需优先使用和风天气，还要准备项目 ID、凭据 ID、专属 API Host 和 Base64 编码的 Ed25519 私钥。
...
不要把真实凭据、地点、坐标、私密来源或运行状态提交到 Git。

## 使用发布镜像

下面的示例使用 Docker Hub 上的固定版本镜像。先创建目录，并复制配置模板：
...
```sh
IMAGE="icecodexi/weather-briefing:1.2.1"
ROOT_DIR="${HOME}/weather-briefing"
...
容器以 UID/GID 65532 的非特权用户运行。上面的 `chown` 通常需要 root 或 sudo 权限。宿主机挂载目录必须允许这个用户写入 `state/`。

无法使用 sudo 时，请用主机提供的权限管理方式让 UID 65532 可以写入该目录。
...
模型调用由 any-llm 处理。不同服务需要的凭据变量以 any-llm 文档为准。官方镜像预装 DeepSeek、OpenAI 和 OpenRouter 所需组件。

RSS 是可选功能。不需要 RSS 时保留空数组即可。启用时，请在私密配置中填写来源名称、URL 和适用地点。
...
```sh
docker exec weather-briefing weather-briefing run forecast --run-now
