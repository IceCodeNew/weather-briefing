...
When a fallback is configured, request failures from the primary provider are retried once with it. Responses that violate the structured output contract stay with the provider that returned them and use the normal bounded repair attempts.
