...
4. 历史上下文的最新值和窗口基线属于最低证据，因预算不足时不得静默丢弃；压缩、降级与告警契约见[系统设计](design.md#状态)。
...
6. 主简报成功投递后，文章处理状态、简报记录、上下文快照、预警更新和待投递权威预报正文必须在一个 SQLite 事务中提交；任一状态写入失败时不得留下部分结果。SQLite 事务不包含外部投递，主简报已被平台接受但结果事务失败时仍可能在后续运行重复，系统不得把该边界描述为端到端 exactly-once。
7. 权威预报正文使用持久队列按稳定顺序独立投递，并且每篇只在平台接受后确认。后续正文失败时，已经确认的正文不得重发；未确认正文必须在下一轮开始时优先重试，且不得因此重复已经 checkpoint 的主简报。平台接受正文后若本地确认失败，队列必须保留并允许后续至少一次重投。
