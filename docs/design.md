...
天气快照可选包含花粉过敏原信息。QWeather 生活指数请求过敏指数（类型 7），并将供应商返回的指数说明直接放入 `lifestyle_advice`。`OpenMeteoProvider` 在同一次空气质量 API 请求中额外请求花粉变量，按浓度分级（参考数据 `allergen_guidance.json`）转换为独立 `AllergenSnapshot`，供每日预报生活建议参考。Open-Meteo 花粉数据仅在欧洲花粉季可用，缺失或字段无效时不影响天气与空气质量结果。当前实现不把综合过敏指数解释为具体花粉种类，也不推断独立的杨絮、柳絮等飞絮信息。
