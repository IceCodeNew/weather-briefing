...
Server酱 Turbo 和 Server酱³ 是两个独立 publisher。Turbo 只读取 `SERVERCHAN_TURBO_SENDKEY`，要求 `SCT...` 格式，并调用 `https://sctapi.ftqq.com/{SendKey}.send`；Server酱³ 只读取 `SERVERCHAN_3_SENDKEY`，要求 `sctp{uid}t{token}` 格式，并调用 `https://{uid}.push.ft07.com/send/{SendKey}.send`。两个适配器都用 JSON POST 发送最长 32 字符且不含换行的 `title` 与支持 Markdown 的 `desp`；Turbo renderer 保留普通 Markdown 换行，Server酱³ renderer 按官方客户端约定把正文行距转换为双换行。HTTP 成功后仍要求 JSON 对象同时包含整数 `code=0` 和字符串 `message`，否则投递失败。常规日志不记录 SendKey、标题、正文或第三方错误正文。
