...
模型返回平台无关的 `BriefingResult`。Telegram 和纯文本渲染器分别负责标题、链接、转义和长度限制。Bark 复用纯文本渲染器。
...
Bark 内容和 APNs 元数据共享 4 KiB payload。为使明文与加密模式行为一致，publisher 将单条消息限制为 650 个可见字符；主简报超过限制时在请求前失败，完整转发等允许分块的内容优先按换行边界切分。INFO 和 WARNING 日志只记录长度、分块位置、HTTP 状态和安全错误类别，不记录 device key、加密 key、正文、ciphertext、IV 或第三方错误正文。HTTP 2xx 响应也必须是 `code=200` 的 JSON，防止把代理或兼容服务的异常响应误判为成功。
