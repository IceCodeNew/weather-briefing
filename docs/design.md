...
Bark 内容和 APNs 元数据共享 4 KiB payload。未显式配置生成限制时，配置入口为 Bark 选择 650 个简报字符，为 Telegram 和 stdout 选择 3500 个字符；LLM 输出 token 默认是有效字符上限的两倍。显式环境变量可以覆盖这些默认值，但 Bark 的字符上限不能超过平台限制。为使明文与加密模式行为一致，Bark publisher 将单条消息限制为 650 个可见字符；主简报超过限制时在请求前失败，完整转发等允许分块的内容优先按换行边界切分。INFO 和 WARNING 日志只记录长度、分块位置、HTTP 状态和安全错误类别，不记录 device key、加密 key、正文、ciphertext、IV 或第三方错误正文。HTTP 2xx 响应也必须是 `code=200` 的 JSON，防止把代理或兼容服务的异常响应误判为成功。
