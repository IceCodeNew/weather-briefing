...
`locations.json` 的每一项都包含稳定 `id`，并提供名称、完整坐标或两者。可选 `language` 是简报目标语言，默认 `en`。日本地点可用 `jma_office_code` 指定六位 JMA 预报区代码。
...
- 只有名称时，依次使用 Open-Meteo Geocoding 和 Nominatim 正向查询；
- 只有坐标时，使用 Nominatim 反向查询；
- 名称和坐标都有时，直接采用配置值。
...
`NEASingaporeNowcastProvider` 只补充新加坡两小时预报。`JMAJapanForecastProvider` 只补充地点所配置预报区的日本本地预报。JMA 没有 office code 时不会猜测东京或其他预报区。
