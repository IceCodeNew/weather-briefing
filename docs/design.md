...
任一已配置 RSS 源重试耗尽会终止任务并增加失败计数。失败计数首次达到阈值时通过投递 provider 发送一次运维提醒，成功任务会重置计数。无更新检测按“最后一次看见该源任意文章”计算，不因当天过滤而丢失活跃信号；同一轮长期无更新只提醒一次，看到更新后才重新开放下一次陈旧提醒。没有 RSS 配置时不会创建 RSS 请求或健康告警，天气 API 继续驱动每小时任务。
