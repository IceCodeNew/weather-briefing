...
新加坡 NEA 作为 capability supplement 接入，只提供两小时 nowcast；JMA 作为日本本地预报 supplement 接入，提供地点配置的 forecast office 官方日/时序预报。两者都不替换主 Open-Meteo 预报。本地 adapter 以 `WeatherContextError` 或输入契约 `ValueError` 报告预期失败时保留主天气上下文，其他编程异常继续暴露；supplement 按配置顺序串行调用并与主 provider 共用 HTTP timeout，不承诺独立的延迟隔离。日本地点必须在 `locations.json` 中按所在预报区配置六位 `jma_office_code` 才会自动追加 `jma-jp`；未配置时只保留 Open-Meteo，避免把东京 `130000` 错用于其他地区。地点国家代码为 `SG` 且未显式配置 provider 顺序时追加 `nea-sg`。显式配置多个 provider 时，`nea-sg`、`jma-jp` 等本地 capability provider 必须形成完整天气 provider 之后的后缀；单独配置时它们可作为唯一上下文来源，但不会因此获得未声明的完整天气能力。
...
- 尚未被成功发布的文章；
- 已发布简报、类别及发布时间；
- 已 checkpoint 但尚未确认投递的权威预报正文、稳定顺序及静默标志；
- LLM 返回的有效预警及最后确认时间；
- 每个 RSS 源最后见到文章的时间；
- 连续任务失败次数。
...
主简报被平台接受后，service 调用状态 adapter 的聚焦结果 checkpoint，在一个 SQLite 事务中写入已处理或待处理文章、简报记录（若已发布）、上下文快照、预警变化和权威预报正文队列。事务任一步失败会回滚全部结果状态。每次任务成功结束时，状态存储再在重置任务失败计数的同一事务中清理历史数据。已处理文章、已发布简报和上下文快照保留 `HISTORY_HOURS`，预警按独立的 `WARNING_RETENTION_HOURS` 保留；恰好位于阈值时刻的记录仍属于当前窗口。尚未成功发布的待处理文章、仍被正文队列引用的文章和使用 upsert 维护的健康状态不参与历史清理。失败任务不清理历史，以免在恢复前丢失诊断与待处理上下文。
...
每轮运行开始时先按持久队列顺序投递此前未确认的权威预报正文。新主简报被平台接受后，结果 checkpoint 把本轮正文加入同一队列，再逐篇投递；每篇只有在平台调用成功返回后才从队列删除，因此中途失败只保留当前及后续正文，已经确认的前项不会重发。静默标志在入队时固化，重试不改变原始投递语义。正文被平台接受但队列确认失败时保留该项，后续运行可能重复投递，属于明确的 at-least-once 边界。
...
**触发条件**：天气上下文获取失败（所有 provider 均不可用）、辅助上下文源获取失败（例如 `HTTPContextSource.fetch()` 抛出 `SourceFetchError`）、LLM 调用或输出校验失败、主简报结果 checkpoint 失败、权威预报正文投递或确认失败、配置校验失败。

**行为**：终止当前任务，并立即通过投递 provider 发送一次性运维提醒。投递失败时在后续任务失败中继续重试；成功投递后，同一失败周期不再重复告警。成功的任务重置计数和告警状态。RSS 获取失败不计入任务失败计数。

主简报投递和 SQLite 结果 checkpoint 无法组成跨系统事务。平台已接受主简报但 checkpoint 失败时，数据库保持事务前状态，后续运行可能重新生成并投递同一主简报；checkpoint 已成功后即使后续正文失败，文章与简报状态也保持完成，下一轮只先重试持久队列，不重复主简报。这个边界提供数据库内部原子性和正文至少一次投递，不承诺端到端 exactly-once。
