...
模型返回平台无关的 `BriefingResult`。Telegram 和纯文本渲染器分别负责标题、链接、转义和长度限制。Bark 使用紧凑纯文本渲染器，正文以短编号关联末尾的来源名称表，不附带来源 URL，但仍保留逐项引用校验。
...
Bark publisher 支持明文推送和可选的 AES-GCM 加密推送。只配置 device key 时，请求体直接包含正文；配置边界要求加密 key 和 IV 同时存在或同时缺失，并允许用不含凭据、query 或 fragment 的绝对 HTTP(S) `BARK_BASE_URL` 覆盖官方端点。加密 key 接受 16、24 或 32 个 ASCII 字符，对应 Bark 的 AES128、AES192 和 AES256；IV 是与 Bark App 设置一致的 12 字符固定值，App 端模式必须设为 GCM 和 noPadding。`cryptography` 的 `AESGCM` 直接生成 Bark 所需的 ciphertext 与 16 字节 tag 组合，再以 Base64 编码。两种模式都使用 `/push` JSON 请求体，避免 device key 进入 HTTP URL 日志。`BARK_GROUP` 原样映射到 `group`；普通投递的内部 level 固定为 `timeSensitive`，无声投递固定为 `passive`。

Bark 内容和 APNs 元数据共享 4 KiB payload。`BRIEFING_MAX_CHARACTERS` 是优先压缩到一条消息的目标，默认且不得超过 650；确有必要时，服务允许简报达到目标的两倍，并最多投递两条。结构化 LLM 输出默认使用 2048 token，以容纳 JSON 字段和来源 ID；显式的较低值仍会传入 prompt，要求模型连同 JSON 结构一并压缩。Telegram 和 stdout 保持 3500 个简报字符和 8192 个输出 token。Bark renderer 省略天气段标题和多余空行，以短编号关联末尾的来源名称表；publisher 使用保证最少分块数的换行优先算法。INFO 和 WARNING 日志只记录长度、分块位置、HTTP 状态和安全错误类别，不记录 device key、加密 key、正文、ciphertext、IV 或第三方错误正文。HTTP 2xx 响应也必须是 `code=200` 的 JSON，防止把代理或兼容服务的异常响应误判为成功。
