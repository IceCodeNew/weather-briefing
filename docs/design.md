...
`OpenMeteoProvider` 提供全球天气，并从独立接口读取空气质量和欧洲花粉数据。它生成固定 `zh-CN` 来源正文，并在生成正文前把 WMO 天气代码转换为可读的中文天气现象；映射数据在模块加载时完成验证，未知代码保留为“未识别天气现象”并写入安全日志。`SourceDocument` 保留这个实际来源语言，模型再把整份来源统一转换为地点的目标语言，因此最终简报不受中文映射限制。公开接口适用于非商业使用，没有 SLA；Base URL 和可选 API Key 可以替换。
