...
正向查询全部失败后，程序根据 `geography.json` 中的规则逐步降低名称精度。每个候选名称仍会走完整的定位服务顺序。
...
镜像提供 `/usr/local/bin/weather-briefing` helper，常驻入口通过它调用虚拟环境内 uv 生成的命令入口。

虚拟环境位于 `PATH` 开头，因此 `docker exec weather-briefing ...` 直接调用同一个 uv 命令入口。两种方式都不需要激活虚拟环境。
