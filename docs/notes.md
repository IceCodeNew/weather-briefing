...
release 工作流的单一 job 只在手动选择 `master` 时运行。它先生成正式版本提交并让同名 Git tag 指向该提交，再生成下一个 patch 的 `-dev` 提交，最后把 master 与 tag atomic push；README 的部署镜像版本只随正式版本提交更新，不随开发版本前移。镜像工作流不提供手动发布或 merge queue 入口；Renovate 更新依赖 branch automerge 后直接触发 `master` 镜像构建，未合并的 Renovate PR 不构建镜像。

开发版本的 `--version` 只在当前目录属于 Git worktree 时附加短 commit SHA，并通过 Git porcelain 状态判断已跟踪和未跟踪的脏工作区。正式版本不探测 Git；Git 不可用或当前目录不属于 worktree 时保留内置 `-dev` 版本。这让已安装的正式包保持可复现版本，同时让源码开发运行报告足够定位代码状态，且不把 Git 作为运行时依赖。
