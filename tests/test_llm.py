...
def test_rejects_markdown_in_headline(markdown_text: str) -> None:
    payload = _valid_payload()
    payload["headline"] = markdown_text

    with pytest.raises(LLMError, match=r"schema validation failed at headline"):
        parse_result(payload, _now(), {"source"})


@pytest.mark.parametrize("plain_text", ("weather_forecast", "3 * 4", "Rain (80%)"))
def test_accepts_plain_text_with_non_markdown_punctuation(plain_text: str) -> None:
    payload = _valid_payload()
    payload["headline"] = plain_text

    result = parse_result(payload, _now(), {"source"})

    assert result.headline == plain_text


@pytest.mark.parametrize(
    ("section", "item"),
    (
        ("conclusions", {"text": "- Rain", "source_ids": ["source"]}),
        ("disaster_tracking", {"text": "# Storm", "source_ids": ["source"]}),
        ("advice", {"topic": "clothing", "text": "**Wear layers**", "source_ids": ["source"]}),
        (
            "active_warnings",
            {
                "id": "warning",
                "title": "[Warning](https://example.invalid)",
                "status": "active",
                "detail": "Details",
                "source_ids": ["source"],
            },
...
def test_rejects_markdown_in_user_facing_sections(section: str, item: object) -> None:
    payload = _valid_payload()
