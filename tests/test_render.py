...
from weather_briefing.delivery import BarkTextRenderer, PlainTextRenderer
...
def test_renderer_fails_when_a_source_reference_is_missing() -> None:
    result = BriefingResult("Daily", ("missing",), ())

    with pytest.raises(KeyError, match="missing"):
        PlainTextRenderer().render_briefing(result, (), ())
...
def test_briefing_sections_follow_the_compact_order() -> None:
    now = pendulum.datetime(2026, 7, 11, 8, tz="Asia/Shanghai")
    ...
    body = PlainTextRenderer().render_briefing(result, (), (context,)).body
...
def test_attribution_deduplicates_only_identical_named_links() -> None:
    context = (
