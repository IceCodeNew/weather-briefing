...
async def test_concurrent_failure_switches_before_another_primary_request() -> None:
    ...
    first = asyncio.create_task(provider.summarize("system", {"attempt": 1}))
    await primary_started.wait()
    second = asyncio.create_task(provider.summarize("system", {"attempt": 2}))
...
async def test_primary_close_cancellation_still_closes_fallback() -> None:
    primary = _provider()
    fallback = _provider()
    cancellation = asyncio.CancelledError()
    primary.aclose.side_effect = cancellation
    provider = FallbackLLMProvider(
        primary,
        fallback,
        primary_name="primary",
...
async def test_fallback_close_failure_does_not_mask_primary_cancellation(caplog) -> None:
    primary = _provider()
    fallback = _provider()
    cancellation = asyncio.CancelledError()
    primary.aclose.side_effect = cancellation
    fallback.aclose.side_effect = RuntimeError("private cleanup detail")
    provider = FallbackLLMProvider(
        primary,
        fallback,
        primary_name="primary",
...
async def test_fallback_close_cancellation_propagates() -> None:
    primary = _provider()
    fallback = _provider()
    cancellation = asyncio.CancelledError()
    fallback.aclose.side_effect = cancellation
    provider = FallbackLLMProvider(
        primary,
        fallback,
        primary_name="primary",
...
async def test_fallback_cancellation_logs_primary_close_failure_type(caplog) -> None:
    ...
    primary.aclose.side_effect = RuntimeError("private primary cleanup detail")
    fallback.aclose.side_effect = cancellation
    provider = FallbackLLMProvider(
        primary,
        fallback,
