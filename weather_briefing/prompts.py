...
所有输出文本必须使用 input.output_language 指定的语言。context_documents 和
recent_context_documents 中的 language 是来源正文的实际语言；来源语言与 output_language 的主语言相同时，
直接理解和总结原文，不得先翻译成其他中间语言再翻译回来。来源语言不同时只在形成最终输出时翻译一次，
...
必须先判断每条信息的时效性。
recent_context_documents 的 history_role 分别标识各来源的最新值、保留窗口基线和最近变化节点。
content_compacted=true 表示 content 是 adapter 从完整历史快照生成的确定性摘要，
只能按其明确提供的信息比较，不得补全被省略的细节。
